...
import { ArrowUpDown, Crosshair, MapPin } from "lucide-react";
import {
  getAddressByCoordinates,
  getAddressSuggestions,
} from "@/entities/geocoding/api/geocoding-api";
import { useCreateIntercityTripMutation } from "@/entities/trips/model/use-trips";
...
export function IntercityTripForm({ isDriver, onCreated, onCancel }) {
  const mutation = useCreateIntercityTripMutation();
  ...
  const onSubmit = async (event) => {
    ...
    await mutation.mutateAsync({
      ...
      createdByDriver: isDriver,
      totalSeats: isDriver
        ? Math.max(1, Math.min(150, Number(totalSeats)))
        : null,
      requiredSeats: isDriver
        ? null
        : Math.max(1, Math.min(150, Number(requiredSeats))),
    });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          {isDriver ? "Создать межгород поездку" : "Создать межгород заявку"}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-2">
              <Label htmlFor="intercity-from-address">Откуда</Label>
          ...
          <div className="space-y-2">
            <Label htmlFor="intercity-to-address">Куда</Label>
            <Input
              id="intercity-to-address"
              value={toAddress}
              onChange={(event) => {
          ...
          <div className="space-y-2">
            <Label htmlFor="intercity-departure-at">Departure date</Label>
            <Input
              id="intercity-departure-at"
              type="datetime-local"
              value={departureAt}
          ...
          <div className="space-y-2">
            <Label htmlFor="intercity-description">Description</Label>
            <textarea
              id="intercity-description"
          ...
          {isDriver ? (
            <div className="space-y-2">
              <Label htmlFor="total-seats">TotalSeats</Label>
              <Input
                id="total-seats"
                type="number"
                min="1"
            ...
            <div className="space-y-2">
              <Label htmlFor="required-seats">RequiredSeats</Label>
              <Input
                id="required-seats"
                type="number"
                min="1"
                max="150"
                value={requiredSeats}
                onChange={(event) => setRequiredSeats(event.target.value)}
                required
              />
            </div>
          )}
